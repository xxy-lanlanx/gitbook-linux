...
socket的数据移动使用核心结构socket缓冲区（sk\_buff）来进行。一个sk\_buff 包含包数据(package data)，和状态数据（state data, 覆盖协议栈的多个层）。每个发送或接收的数据包都用一个sk\_buff来表示。该sk\_buff 结构是在linux/include/linux/skbuff.h中定义的，并在图4中示出。
