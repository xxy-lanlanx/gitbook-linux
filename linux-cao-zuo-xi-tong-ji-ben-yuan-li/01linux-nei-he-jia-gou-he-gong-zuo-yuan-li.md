...
> 本篇是"Linux 操作系统基本原理"部分的开篇，先建立整体认知：什么是操作系统与内核、Linux 内核属于哪一类、它的层次如何，以及"如何配合《编程基本功》部分一起学"。后续 02 起进入内核整体架构与各子系统的细节。
...
> 这条"用户态 → 系统调用 → 子系统 → 硬件"的链路，会贯穿全书。每往下一层，你就更接近"编程基本功"里的 C/汇编/计算机系统基础。
...
内核源码里大量使用指针、宏、位运算、链表、系统调用——这些"编程基本功"如果不扎实，读源码会非常吃力。建议：

- 先读《[编程基本功总览](../bian-cheng-ji-ben-gong/01-bian-cheng-ji-ben-gong-zong-lan.md)》，建立五大支柱（C/数据结构/工具链/计算机系统/规范）的能力地图；
- 遇到看不懂的代码构造（如 `container_of`、`set_bit`、`htonl`），回到对应支柱章节把最小示例跑通；
- 每篇原理文章末尾都有 **「编程基本功实践」** 板块，给出可运行代码与自测题。
